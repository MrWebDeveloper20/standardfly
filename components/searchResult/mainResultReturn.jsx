...
const MainResult = (props) => {
  ...
  const [FlightNoG, setFlightNoG] = useState(props.FlightNo)
  const [timeD, settimeD] = useState(props.timeD)
  const [dateG, setdateG] = useState(props.dateG)
  ...
  useEffect(() => {
    setLoading(true)
    const res = fetch(
      `/api/avail?from=${props.to} &to=${props.from} &day=${props.day2} &monthNum=${props.monthNum2}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="flex w-full flex-col h-fit z-20 mb-10 justify-center items-center p-5 bg-slate-100">
      ...
      <div className="w-full flex flex-col justify-center items-center lg:w-72 shadow-black shadow-md">
        <h2 className="alib text-red-500 text-xl mb-2 mt-3"> بلیط رفت </h2>
        <h2 className="alib text-slate-900 text-xl mb-2">
          {' '}
          تاریخ : {day} {monthName}
        </h2>
        <h2 className="alib text-blue-600 text-xl mb-2">
          {' '}
          {fromname} به {toname}{' '}
        </h2>
        <h2 className="alib text-slate-900 text-xl mb-2">
          {' '}
          شماره پرواز : {FlightNoG}{' '}
        </h2>
        <h2 className="alib text-green-600 text-xl mb-3">
          {' '}
          ساعت حرکت : {timeD}
        </h2>
      </div>
          ...
          >
            ...
            {isOpenInfo || isOpenReturn ? (
              <div className="ticketLeft flex flex-col lg:h-96 w-1/5 justify-center items-center bg-white hover:bg-yellow-100 ring-1">
                ...
                <Link
                  href={{
                    pathname: `/flight2/${FlightNoG}/passengers/`,
                    query: {
                      from: from,
                      fromname: fromname,
                      to: to,
                      toname: toname,
                      flightNoR: dt.OperatingFlightNo,
                      FlightNoG: FlightNoG,
                      adult: adult,
                      child: child,
                      infant: infant,
                      ...
                      timeD: timeD,
                      dateR: dt.DepartureDateTime.toString().slice(0, 9),
                      dateG: dateG,
                    },
                  }}
