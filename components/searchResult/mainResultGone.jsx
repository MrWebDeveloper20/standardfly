...
const MainResult = (props) => {
  ...
  const [day2, setday2] = useState(props.day2)
  const [monthNum2, setmonthNum2] = useState(props.monthNum2)
  const [monthName2, setmonthName2] = useState(props.monthName2)
  const [year2, setyear2] = useState(props.year2)
    ...
    <div className="flex w-full flex-col h-fit z-20 mb-10 justify-start p-5 bg-slate-100">
      ...
      {length > 0 && (
        <div className="w-full flex flex-col justify-center items-center">
          <h2 className="alib text-red-500 text-xl mb-2">
            انتخاب بلیط رفت {day} {monthName}
          </h2>
          <h2 className="alib text-blue-500 text-xl">
            {' '}
            در مرحله بعد می توانید بلیط برگشت را انتخاب کنید{' '}
          </h2>
        </div>
      )}
            ...
            {isOpenInfo || isOpenReturn ? (
              ...
              <div className="ticketLeft flex flex-col lg:h-56 w-1/5 justify-center items-center bg-white hover:bg-yellow-100 ring-1">
                ...
                <form action="/flight2/return">
                  ...
                  <input type="hidden" name="MName" value={monthName} />
                  <input type="hidden" name="MNum" value={monthNum} />
                  <input type="hidden" name="year" value={year} />
                  <input type="hidden" name="day2" value={day2} />
                  <input type="hidden" name="MName2" value={monthName2} />
                  <input type="hidden" name="MNum2" value={monthNum2} />
                  <input type="hidden" name="year2" value={year2} />
                  <input type="hidden" name="azz" value={from} />
                  <input type="hidden" name="bee" value={to} />
                  <input
                    type="hidden"
                    name="timeD"
                    value={dt.DepartureDateTime.toString().slice(11, 16)}
                  />
                  <input
                    type="hidden"
                    name="dateG"
                    value={dt.DepartureDateTime.toString().slice(0, 9)}
                  />
                  {/* <input type="hidden" name="timeA" value={dt.ArrivalDateTime.toString().slice(11, 16)}/> */}
                  <input
                    type="hidden"
                    name="FlightNo"
                    value={dt.OperatingFlightNo}
                  />

                  <button
                    className="bg-blue-600 alib my-4 text-lg px-10 py-4 text-white rounded-lg"
                    type="submit"
                  >
                    انتخاب پرواز رفت
                  </button>
